...
while (1):
    ...
    for data_integrasi in integrasi:
        ...
        # get update
        elif ((data_integrasi[7] == 'update') and (data_integrasi[2] == config.KODE_TOKO) and (data_integrasi[9] == 'toko')):
            for data_transaksi in transaksi:
                if (data_integrasi[1] == data_transaksi[1]):
                    # print("Update id %s toko %s" % (data_integrasi[1], config.KODE_TOKO))
                    data = (data_integrasi[3], data_integrasi[4], data_integrasi[5], data_integrasi[1], config.KODE_TOKO)
